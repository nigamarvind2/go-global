...
const StudyAbroad = () => {
  ...
  const itemListJsonLd = {
    ...
    itemListElement: countries.map((country, index) => ({
      "@type": "ListItem",
      position: index + 1,
      item: { "@type": "Country", name: country.name, url: `${origin}/study-abroad/${country.slug}` },
    })),
  };
  const faqJsonLd = {
    "@context": "https://schema.org",
  ...
  return (
    <>
      ...
      <PageHeader
        title="Study Abroad Destinations"
        subtitle="Our recruiting efforts span across the globe. Our skilled and knowledgeable team is here to help you explore your desired study destinations."
        breadcrumbs={[
          { label: "Home", href: "/" },
          { label: "Study Abroad" },
        ]}
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {country.highlights.map((highlight) => (
                    <span key={highlight} className="text-[11px] bg-primary/8 text-primary px-2 py-0.5 rounded-md font-medium">
                      {highlight}
                    </span>
                  ))}
                </div>
                <div className="flex items-center text-primary text-xs font-medium group-hover:gap-1.5 transition-all">
                  Learn More
      ...
      {/* FAQ Section */}
      <div className="px-6 md:px-8 py-10 border-t border-border/60">
        <div className="mb-6">
          <span className="eyebrow">FAQs</span>
          <h2 className="section-title mt-1.5 mb-2">Study Abroad Questions</h2>
